        ...
        <div class="layui-inline" style="display: none">
            ...
            <div class="layui-input-inline" style="padding: 5px">
                <input type="text" name="identity" autocomplete="off" class="layui-input layui-input-inline"
                       placeholder="请输入身份证号" style="height: 30px;border-radius: 10px" value="${user.identity}">
            </div>
        </div>
        <div class="layui-inline">
...
<div id="rentToolBar" style="display: none;">
    <button type="button" class="layui-btn layui-btn-danger layui-btn-sm layui-btn-radius" lay-event="deleteManyBatch" id="deleteManyRent">批量删除</button>
    <button type="button" class="layui-btn layui-btn-danger layui-btn-sm layui-btn-radius" lay-event="addRentBatch" id="addManyRent">批量添加</button>
</div>
    ...
    <form class="layui-form" lay-filter="dataFrm" id="dataFrm">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline" >
                    <input type="text" id="price" name="price" lay-verify="required" placeholder="请输入出租价格" class="layui-input" disabled>
                </div>
            </div>
        </div>
        <div class="layui-form-item">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${alfred}/rent/loadCustomerBusRentDetail.action?identity='+${user.identity} //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "${alfred}/rent/loadCustomerBusRentDetail.action?" + params,
                page: {curr: 1}
            })
        });
            ...
            table.on("toolbar(rentTable)",function (obj) {
                ...
                if (layEvent === 'deleteManyBatch'){
                    ...
                    $.each(data,function (i, item) {
                        if (i==0){
                            params+="rentIds="+item.rentid;
                        }else {
                            params+="&rentIds="+item.rentid;
                        }
                    });
                    layer.confirm('真的要删除这些待租赁订单吗？',function (index) {
                        //向服务端发送删除指令
                        $.post("${alfred}/customerRent/deleteManyCustomerBusRent.action",params,function (res) {
                            layer.msg(res.msg);
                            tableIns.reload();
                        })
                    })
            ...
            table.on("toolbar(rentTable)",function (obj) {
                ...
                if (layEvent === 'addRentBatch'){
                    ...
                    layer.confirm('真的要添加这些待。。。。。吗？',function (index) {
                        ...
                        $.ajax({
                            type:'post',
                            url:'${alfred}/customerRent/addCustomerBusRent.action',
                            data:{
                                dataMsg:obj
                            },
       ...
       //批量添加用户个人待租赁列表到用户个人订单
        ...
        table.on('tool(rentTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.rentid + '】这个“待”出租单么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${alfred}/customerRent/deleteCustomerBusRent.action", {rentid: data.rentid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
                ...
                window.location.href="${alfred}/customerBusRent/exportRent.action?rentid="+data.rentid;
            }
        });
        ...
        function openUpdateRent(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${alfred}/customerRent/updateCustomerBusRent.action";
                }
            });
        }
